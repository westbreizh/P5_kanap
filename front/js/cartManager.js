/** 
 * fonction de modification de la valeur de la propriété quantity de l'objet kanapLS dans le local storage au click sur le bouton 
...
function changeQuantity(id, color, quantity) {
    ...
    for (let kanapLS of array) {  // pour chaque canapé dans le local storage
        if (kanapLS._id == id && kanapLS.color == color) { // on vérifie si il correspond au canapé dont la quantité est modifié par l'utilisateur
            kanapLS.quantity = quantity; // on affecte la nouvelle quantitée
...
function deletCanapInlocalStorage(id, color) {
    ...
    for (let kanapLS of array) {  // on récupère le bon canapé du local storage qui correspond à celui dont la qauntité a été changé par l 'utilisateur
        if (kanapLS._id == id && kanapLS.color == color) {
            let index = array.indexOf(kanapLS); // on récupère l'index, la position du canapé dans le tableau
...
function calculAndInsertTotalQuantity() {
    ...
    for (let kanapLS of array) {  // pour chaque objet kanapLS du tableau du localstorage
        let quantity = parseInt (kanapLS.quantity); //on récupère la qauntité
